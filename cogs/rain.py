...
class Rain:
    def __init__(self, bot):
        self.bot = bot
        soak_config = parsing.parse_json('config.json')['rain']
        '''
    ...
    async def rain(self, ctx, amount: float):
        """
        Tip all online and offline users
        """
        ...
        online_users = [x for x in ctx.message.server.members if (x.status != discord.Status.invisible and not x.bot) ]
        ...
        if amount_split == 0:
            await self.bot.say("{} **:warning: {} REDN is not enough to split between {} users! :warning:**".format(ctx.message.author.mention, amount, len_receivers))
            return
        receivers = []
